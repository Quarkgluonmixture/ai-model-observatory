...
- [ ] ⭐ **要你定:早批次占坑的那批参数格,要不要整批推到最新读数。**
      ...
      - 我的倾向:**intelligence 与 costTask 整批推到 batch-14,speed / latency 不动**。
        ⭐ **2026-08-12 补测,理由比「过期」强得多 —— 是两把尺子混在一列里**:
        逐格比对手抄批次(06/07/08)与 batch-14,方向**不是随机的**:

        | 字段 | 上升 | 下降 | 基本不变(±2%) | 读法 |
        |---|---|---|---|---|
        | `intelligence_index` | **26** | **0** | 11 | 全体单向 ⇒ 换了标度,不是噪声 |
        | `cost_per_task_usd` | 2 | **17** | 13 | 强单向 ⇒ 口径变了 |
        | `output_tokens_per_s` | 14 | 5 | 1 | 双向 ⇒ 活属性抖动 |
        | `latency_first_chunk_s` | 4 | 12 | 4 | 双向 ⇒ 同上 |

        26 涨 0 跌不可能是重测噪声。batch-14 自报 **intelligence index v4.1**,而
        `batch-06/07/08` 的行**根本没记版本号**(2026-07-31/08-01 手抄)⇒ 无法证明是哪一版,
        但能证明**不是同一版**。除 glm-5.2 外全部只动 1–2.5 分,glm-5.2 动 **18.6**(34 → 52.6),
        那一条是单独的异常,要单独看,别混进标度问题里。
        ⇒ 所以这不只是「站上挂着旧数」,是**通用能力排名正在拿两把尺子互相比**。
        ⚠ 而且**这个混合每加一个新模型就大一格**:新模型没有老批次的行,batch-14 直接获胜 ⇒
        新记录一律落在新标度上(`qwen3.8-max` 的 58.1 就是 8-12 刚加进去的一条)。
        ⇒⇒ speed / latency 双向抖动,维持不动的判断不变。
