...
## ⭐ 要你定(2026-09-04 体检后剩下的)

自动化这一轮**只解决了「重复劳动」那类**。下面四件都是**真的要人**,不是漏做的自动化 ——
每条都写了它卡在哪一个条件上,判完就删。

- [ ] ⭐ **PR #108**(CAIS HLE batch-38):**条件 2 不满足 —— 18 个已发布的 hle-no-tools 数字会变**。
      语义上是升级(出题方自己的读数按规则 3 优先于第三方 AA 行),但「站上已有的数字变了」按设计
      就是 owner 的 call。⇒ 要么合(接受这批下修,Opus 5 54.9→51 这类),要么说不要。
- [ ] ⭐ **要你定:issue #122 —— LiveBench 改写了冻结版本下的 3 个数,归档该怎么说。**
      2026-06-25 冻结版下 3 个 nemotron 的 mini-SWE-agent 分被改高(27.273→54.545 / 15→45 /
      6.667→16.667)。23 条 withdrawn 已在 `withdrawnRows` 里承认、不失败,**只有这 3 条 changed 让它红**。
      ⭐ **先读这条再判**:`nemotron` **不在目录里**,那 3 格**一格都没进 `observations.generated.ts`**
      ⇒ 站上看不见,这是纯归档记账题,不是数据事故(实测 = `grep -c nemotron app/model-data.ts` → 0)。
      两条路,都要人:**(a)** 给「上游改写了值」造一个 `withdrawnRows` 的同类承认位
      (那是**改整个完整性契约**,不是维护性修改);**(b)** 判 LiveBench 这个 release 其实不该声明成
      冻结,改 fetcher 的 `versioning` 一行。⛔ 别 re-fetch、别手改归档值(硬规则 2)。
      ⚠ 与它**无关**的两件已解耦:红心跳的语义已在章程说死;可用性红已不再借这个 job 的颜色。
...
- [ ] ⭐ **要你定(还没做,是这次体检的真正结论):新模型进不了目录不是 bug,是地板在起作用。**
      闸门每天跑、**一次都没触发过**(`auto/new-model` 零 PR;上次加模型 = 2026-08-06 人工)。
      地板 = 现有模型平均格数 = **49**,而今天最好的候选只有 31 格 ⇒ 结构性够不到。
      放开地板已在 8-12 判过「不放开」(会自我强化地塌),⛔ 别重新论证。
      ⇒ 真正的出路是下面那条**「最新发布」看板** —— 它们本来就不在覆盖率分母里,一个点都不掉。
