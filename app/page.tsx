...
function BenchmarkLines({items,lang}:{items:ModelRecord[];lang:Lang}){
  const metrics=[
    {label:lang==="zh"?"AA 智能":"AA Intelligence",unit:"index",raw:(m:ModelRecord)=>m.intelligence,score:(m:ModelRecord)=>clamp(m.intelligence/65*100)},
    {label:lang==="zh"?"Arena 文本":"Arena Text",unit:"Elo",raw:(m:ModelRecord)=>m.textElo,score:(m:ModelRecord)=>m.textElo===null?null:clamp((m.textElo-1320)/2.3)},
    {label:"Code Arena",unit:"Elo",raw:(m:ModelRecord)=>m.codeElo,score:(m:ModelRecord)=>m.codeElo===null?null:clamp((m.codeElo-1400)/3.5)},
    {label:lang==="zh"?"输出速度":"Output speed",unit:"tok/s",raw:(m:ModelRecord)=>m.speed,score:(m:ModelRecord)=>m.speed===null?null:clamp(m.speed/230*100)},
    {label:lang==="zh"?"首包延迟":"First chunk",unit:"s · lower",raw:(m:ModelRecord)=>m.latency,score:(m:ModelRecord)=>m.latency===null?null:clamp((1-m.latency/170)*100)},
    {label:lang==="zh"?"单任务成本":"Cost / task",unit:"$ · lower",raw:(m:ModelRecord)=>m.costTask,score:(m:ModelRecord)=>clamp((1-m.costTask/3.5)*100)},
  ];
  const x=(i:number)=>84+i*143;
  const y=(score:number)=>25+(100-score)*1.72;
  const display=(raw:number|null,unit:string)=>raw===null?"N/A":unit.startsWith("$")?`$${raw.toFixed(2)}`:unit.startsWith("s")?`${raw.toFixed(2)}s`:raw.toFixed(0);
  return <div className="benchmark-lines"><div className="chart-wrap"><svg className="benchmark-chart" viewBox="0 0 900 245" role="img" aria-label={lang==="zh"?"Benchmark 归一化折线对比":"Normalized benchmark line comparison"}>
    {[0,25,50,75,100].map(v=><g className="line-grid" key={v}><line x1="58" x2="830" y1={y(v)} y2={y(v)}/><text x="48" y={y(v)} textAnchor="end" dominantBaseline="middle">{v}</text></g>)}
    {metrics.map((m,i)=><text className="axis-label" key={m.label} x={x(i)} y="224" textAnchor="middle">{m.label}</text>)}
    {items.map(model=>{const points=metrics.map((m,i)=>{const score=m.score(model);return score===null?null:{x:x(i),y:y(score),score,raw:m.raw(model),metric:m}});return <g className="bench-series" key={model.id}><polyline points={points.filter(Boolean).map(p=>`${p!.x},${p!.y}`).join(" ")} fill="none" stroke={model.color}/>{points.map((p,i)=>p&&<circle key={i} cx={p.x} cy={p.y} r="4" fill={model.color}><title>{model.name} · {p.metric.label}: {display(p.raw,p.metric.unit)}</title></circle>)}</g>})}
  </svg></div><div className="benchmark-values">{metrics.map(metric=><div key={metric.label}><span>{metric.label}</span><small>{metric.unit}</small>{items.map(model=><b key={model.id} style={{color:model.color}}><i style={{background:model.color}}/>{display(metric.raw(model),metric.unit)}</b>)}</div>)}</div></div>
}
