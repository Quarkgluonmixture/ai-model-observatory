...
| 你要动 | 先读 |
|---|---|
| DeepSeek 任何东西 | **24**(preview→GA 串号:判据 + 动手顺序)· **25/26**("preview" 在名字里 ≠ 是个 preview;引用别人写的格数前先自己数) |
| 价格 / 参数批次 | **27**(`retrievedDate` 只对页面快照等于测量日期)· **37**(改了目录价就要同一次退休那条 term,否则它开始断言反面) |
| workflow / 通知 / 脚本封顶 | **29**(报警器在正比于它要报的规模上失灵,而失灵的样子是绿色)· **38**(报警的单位是状态变化,不是一次运行;沉默 = 没变,不是绿了) |
