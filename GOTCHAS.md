...
| 你要动 | 先读 |
|---|---|
| DeepSeek 任何东西 | **24**(preview→GA 串号:判据 + 动手顺序)· **25/26**("preview" 在名字里 ≠ 是个 preview;引用别人写的格数前先自己数)· **40**(地板是给新记录用的,卡不了身份翻转) |
...
| 采一张厂商发布表 | **33**(别的厂商的表可能已经填过那些格)· **36**(发布帖发的是图 ≠ 这张表没有机器可读的副本) |
| 写任何基于 diff 文本的判据 | **34**(闸门自己也会够不到) |
| 判断「某个源/某个模型在等刷新」 | **41**(drift 读与 refresh 写是两个 job;还活着的只读通道能直接回答) |
