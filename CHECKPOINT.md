...
⭐⭐ **同日 owner 一次裁决三道题,全部封口**(理由 → `LOG.md` 2026-09-04 第三条):
**issue #122 不管**(「改了就改了」)· **「最新发布」看板不做** · **hermes 死了不推微信**。
⇒ `TODO.md` 顶部**从此没有「要你定」**。

⚠⚠ **由此产生的一条必须先知道的现场状态**:**每日 drift job 会长期红,#122 会长期开着,
⛔⛔ 别去关它**(关了明天重开,而「开」会再推一次微信;挂着不动是静默原地编辑)。
⇒ **这个 job 的红 = 「就是这一条」,不再是一个可用的新信号** —— 下次真出别的 integrity 问题
看到的还是同一片红,要读 issue body 的 cell 清单分辨。全文 → 坑 **54**。
