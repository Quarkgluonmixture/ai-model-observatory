...
观测 2141 → **2159**,被审计的目录值 321 → **325**。⚠ 三个**已发布**观测数字被移动
...
**现在要你定的是它挖出来的那件更大的**(`TODO.md` 小口子第一条):`check:models` 每格取
**第一个**给出它的行、档案按文件名读 ⇒ 手抄的 `batch-06/07/08` 永远压着脚本的 `batch-14`,
**早批次冻住的值不管源漂多远都还是「backed」**。已动一格(`qwen3.7-max` cost 1.28→0.5413,
你点头的那个),**其余 34 格没动**。数量以 `npm run check:models` 尾部那节为准,别抄文档里的数。

⚠⚠ 通用判据(这次又验证一遍):**「契约全绿」只覆盖检查够得到的东西** —— 解析不到的行被
`check-model-provenance.mjs` 直接 `continue`,所以 321/321 的**范围是解析得到的行**。
...
| 溯源 | **325 / 325**（100%；8-12 由 Qwen Max 解析修复 +4）⚠ 分母是**解析得到的行**，见下 |
...
5. **`check:models` 尾部那节新报告**(8-12 起):早批次占坑且与最新读数差 >5% 的参数格。
   它**不失败**——首次打印 35 格。整批推不推是 `TODO.md` 小口子第一条,要你定。
...
| `GOTCHAS.md` | **动手之前扫一遍**：22 条仍会咬人的坑，编号稳定可引用。⭐ 8-12 新增第六族「审计的取数口径本身就是盲区」(21 = 厂商产品档 vs 操作点写在同一个后缀位、22 = 参数四格先到先得)。⭐ 8-10 新增第五族「我以为我知道数据长什么样」(15–18)——一天踩三次同形错；19 = 规则写在报告里≠网站在生效(同一句话两条代码路径)，20 = `<small>` 自己掉到 9px 底下 + 检查器不选中的元素永远通过 |
