...
**接手点** — 2026-08-13:**preview/GA 串号这件事的机制侧做完了,只剩一个命名决定给你。**
分界线是:**能全自动的是「发现 + 拒收」,不是「命名」** —— 危险从来不在于没人处理,
而在于处理之前它会**静默写错**;改成大声缺,时间压力就没了。三件都进 CI、都能单独回滚:
① 归属闸门第五条拒绝(跨文件的**带日期兄弟串**,补上"源自己不改 slug"那个形状);
② `tags` 里 `open weights` / `preview` 改成从记录**派生**,手写抛错;
③ **`modelWindows`** —— 记录级测量窗口,`deepseek-v4-pro` 挂 `validUntil: 2026-08-12`。
③ 是唯一真正的敞口:**十条**(不是 24 里说的五条)`effort:"*"` alias 指向这条 preview 记录。
挂在**记录**上而非 alias 上,因为 cutover 是一个事实,写十份副本还会漏掉**闸门明天新写的第十一条**。
⭐ **实测:`npm run ingest` 逐字节不变** —— 今天零影响,是纯守卫。细节与三组实测在 `LOG.md` 2026-08-13。

⚠ **两条纠正,都在 `GOTCHAS.md` 新条**:
**25** ——「preview 不单独收记录」这条规矩**不能按显示名执行**:名字带 Preview 的还有
`gemini-3.1-pro`(**59 格,全目录第二满**),Google 把 Preview 当在售版本卖。判据是
**上游有没有一个取代它的 GA**。差点据此退掉一条 59 格的记录,动手前量了一下才没错。
**26** —— 本文件与 24 反复引用的「`deepseek-v4-pro` 的 11 格」是**四月 model card 那部分**,
整条记录实测 **58 格**。结论不变(那些三方读数全在 GA 之前,量的仍是 preview),但拿 11 去
估影响面会差 5 倍。
⇒ **要你定的两件在 `TODO.md`**(preview 该不该有自己的记录 · 窗口何时解除)。
两件都得**等第一块板真发 GA 分**才能动 —— 第一个源决定串怎么拼。厂商表入档仍卡在 `source_url`。

⏰ **有硬期限了(第二轮):2026-08-16 16:00 UTC,DeepSeek 换峰谷计价。** v4-pro 输入 0.435→**1.32**、
输出 0.87→**3.96**(峰时;谷时是峰时半价,**仍比现价贵一倍以上**)。已落档 `batch-31` +
`priceTerms` 的新形状 `scheduled`,**契约那天会自己红**(实测 8-15 exit 0 / 8-16 exit 1),而
`check:prices` 是日常 job 自动合并的闸门 ⇒ 那天宁可停发,不发错价。
⚠⚠ **那天别顺手把新价填进去**:定价页只印裸串、**不印 `-0813`**,按 24 判据 2 这两个价属于 **GA**,
而目录那条是 **preview**。**先定记录身份,价格跟着身份走**;身份没定就**让它红着**。
⭐ 顺带查明两件结构性的事(细节在 `LOG.md` 2026-08-13 第二轮):**价格从来不进 ingest**
(参数循环只收 Elo,站上价格是手打的);**厂商价格这条链连第一米都没有**(唯一的官方 DeepSeek
价格行是 8-01 手抄的),而审计**优先采信官方源** ⇒ **被优先采信的恰好是不会更新的那份**。
已定:AA 不当权威(它会把档位结构压成一个数),改让它当**漂移探针** —— 待做,在 `TODO.md`。

**收录地板:已定「不放开」**。它不是写死的数,是当前每模型平均格数(8-13 实测 1400÷29=48.3;
复算 = `npm run propose:model` 头几行会打印它),
